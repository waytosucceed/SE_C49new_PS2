...
fetch('questions.json')
  ...
  .then(data => {
    ...
    if (selectedTopic) {
      ...
      // Load previously saved answers and submitted questions
      Ansgiven = getFromLocalStorage('Ansgiven') || [];
      submittedQuestions = getFromLocalStorage('submittedQuestions') || [];
...
function loadQuestion(index) {
  ...
  randomQuestion.options.forEach(function(option, idx) {
    ...
    if (option.image) {
      ...
      optionImage.style.width = "90%"; // Adjust width to fit two images per row
optionImage.style.maxWidth = "230px"; // Optional max width
